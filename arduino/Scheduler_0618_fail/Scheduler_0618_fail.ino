...
void setup()
{
  ...
  delay(100);
   ...
   strip.show();
    ...
    matrix.drawCircle(32,16,4,matrix.Color333(0,2,4));
...
void tcpInterface(boolean scheduleOnoff) {
  ...
  if(scheduleOnoff) {
   ...
   Serial.println("parsingStr");
   parsingStr();
   Serial.println("parsingClock");
   parsingClock(toStringData(&REALTIME));
   Serial.println("showParsingData");
   ShowParsingData(); //삭제예정
   Serial.println("led Control");
   ledControl();
   Serial.println("setWeather");
   setWeather(); //text 추가
