...
#define LED_S_BRIGHT 60             // led strip 밝기
...
color setColor(char whatColor) { //0-23
  ...
  switch(whatColor/12) { //밝기 조절 필요
      ...
      Serial.print(wc.r,DEC);
      Serial.print(wc.g,DEC);
      Serial.println(wc.b,DEC);
      return wc;
}
  ...
  setWeather();
   ...
   for(int i=0;i<240;i+=5) {
    whatColor = setColor(i/10);
     ...
     Serial.print(whatColor.r,DEC);
     Serial.print(" g:");
     Serial.print(whatColor.g,DEC);
     Serial.print(" b:");
     Serial.println(whatColor.b,DEC);
...
void watch(int hour,int minute){ // 시계함수
  ...
  matrix.print(hour,DEC); // 서버에서 받아온 현재시간을 변수화시켜서 출력
